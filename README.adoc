...
|hybrid_cloud_dist_clean | Delete the out directory |  False |
...
After successful provisioning of <<prov-loud-resource>>, there should be one directory per cloud created under `$PROJECT_HOME/out`.

Each cloud directory has as `deploy.sh` which can used to deploy the OpenShift to the respective clouds.

e.g. lets say you have `gcp` in your clouds, the following steps shows how to deploy OpenShift on to your cloud:
